function [C,np]=stcovgui(Zi,cMSi,tMEi,Zj,cMSj,tMEj,rLag,rLagTol,tLag,tLagTol,handles,dist,method);
...
%
[idxpairsMS]=pairsindexgui(cMSi,cMSj,rLag,rLagTol,dist,method,handles);
...
for ir=1:nr
  ...
  if nMSpairs==0,
    C(ir,1:nt)=NaN;
    np(ir,1:nt)=0;
    displayString = sprintf('Calculating experimental covariance. Please wait... %5.2f %%',ir/nr*100/2+50);
    ...
    for it=1:nt,
      displayString = sprintf('Calculating experimental covariance. Please wait... %5.2f %%',(((ir-1)*nt)+it)/(nr*nt)*100/2+50);
      set(handles.feedbackEdit,'String',displayString);
      pause(0.01);
