function [iu,ir]=findneardupli(p,gridSpan,proxLimit)
%  findneardupli         - find nearly duplicate coordinates (closer than 0.1%)             
%
%  Find in a set of points those that are almost duplicates (coordinates closer than 0.1%)
%
%  SYNTAX :
% 
%  [iu,ir]=findneardupli(p);
% 
%  INPUT :
%
%  p          n by d     matrix of the coordinates of n points in a space of dimension d
%  proxLimit  1 by 1     scalar that designates the limit below which data are
%                        considered as co-located
%  gridSpan   1 by d     vector of grid size in each of the d directions used
% 
%  OUTPUT :
%
%  iu        nu by 1    vector with the indices of the points that are not in duplicates
%                       When there are no duplicates, then p(iu,:) is equal to p
%                       otherwise nu<n and p(iu,:) is the subset of p that do not have duplicate
%                       coordinates
%  ir        1 by nr    cell array of the duplicate points.  
%                       When there are no duplicates, then ir is empty
%                       Otherwise ir{k} is a k=th cluster of duplicate points, so that
%                       p(ir{k},:) is the k-th subset of p having all the same coordinates

iu=[1:size(p,1)]';
[n d]=size(p);
i = (1:n)';
spi = sortrows([p i],1:d);
ps = spi(:,1:d);
is = spi(:,d+1);
%ind = abs((ps(2:end,1) - ps(1:end-1,1))./ps(2:end,1)) < proxLimit;
ind = abs((ps(2:end,1) - ps(1:end-1,1))./gridSpan(1)) < proxLimit;
for k=2:d
  %ind = ind & ( abs((ps(2:end,k) - ps(1:end-1,k))./ps(2:end,k)) < proxLimit );
  ind = ind & ( abs((ps(2:end,k) - ps(1:end-1,k))./gridSpan(k)) < proxLimit );
end
ind = [0; ind; 0];
if sum(ind)            % If there are non-zero elements, i.e., if there are near duplicates
  fs = find(ind(1:end-1) == 0 & ind(2:end) == 1);
  fe = find(ind(1:end-1) == 1 & ind(2:end) == 0);
  irv=[];
  for k = 1 : length(fs)
    ir{k}=is(fs(k):fe(k));
    irv=[irv;ir{k}];
  end
  iu=setdiff(iu,irv);
  if isempty(iu), iu=[]; end
else
  ir = [];
end